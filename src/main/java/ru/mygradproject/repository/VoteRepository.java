...
public interface VoteRepository extends JpaRepository<Vote, Integer> {
    ...
    @Query("SELECT v FROM Vote v WHERE v.primaryKey.user.id=:userId AND v.primaryKey.date=:currentDate")
    Vote getByUserIdAndDate(@Param("userId") int userId, @Param("currentDate") LocalDate localDate);

    @SuppressWarnings("JpaQlInspection")
    @Query("SELECT v FROM Vote v WHERE v.restaurant.id=:restaurantId AND v.primaryKey.date BETWEEN :startDate AND :endDate")
    ...
    @Query("SELECT v FROM Vote v WHERE v.primaryKey.user.id=:userId")
    List<Vote> findAllByUserId(@Param("userId") int userId);

    @Query("SELECT v FROM Vote v WHERE v.restaurant.id=:restaurantId AND v.primaryKey.date=:date")
